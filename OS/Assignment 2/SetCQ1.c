...
void display_lines(const char *filename, int n, int mode) {
    ...
    char line[1024]; int total_lines = 0, skip_lines = 0;
    if (mode == 1) { while (fgets(line, sizeof(line), file)) total_lines++; fseek(file, 0, SEEK_SET); skip_lines = total_lines > n ? total_lines - n : 0; }
    for (int i = 0; mode == 0 ? i < n && fgets(line, sizeof(line), file) : fgets(line, sizeof(line), file); i++) if (mode == 2 || (mode == 0 && i < n) || (mode == 1 && i >= skip_lines)) printf("%s", line);
    fclose(file);
}
    ...
    while (1) {
        ...
        if (pid == -1) { perror("fork"); exit(1); }
            ...
            if (args[0] && !strcmp(args[0], "typeline")) {
                ...
                } else printf("Usage: typeline <n|-n|a> <filename>\n");
                exit(0);
            } else { execvp(args[0], args); perror("execvp failed"); exit(1); }
        } else wait(NULL);
    }
    return 0;
}
