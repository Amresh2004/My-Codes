...
void addlist(int page) 
{
	...
	{
            if (future_use[frame_idx] == -1) 
	    {  // This page will never be used again
                farthest = frame_idx;
                break;
            } 
	    else if (future_use[frame_idx] > farthest) 
        ...
        if (index == -1) 
	{  // If a page will never be used again, replace it
            temp = start;
            start = start->next;
            free(temp);
            last->next = ptr;
	...
	else 
	{
	    ...
	    { // Traverse to the index that needs to be replaced
                prev = temp;
                temp = temp->next;
            }
            if (prev != NULL) 
